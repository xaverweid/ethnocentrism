...
to setup
  ...
  crt num-agents [
    ...
    let strategy-list []
    repeat n-groups [
      set strategy-list lput one-of ["C" "D"] strategy-list
    ]
  ...
  ask links [
    set had-game? false
    set used-strategies ["N" "N"] ;; N stands for no strategies used
  ]
...
to go
  ...
  ask links [
    ...
    ifelse (random-float 1 < min list likeness01 likeness10)[
      ...
      set used-strategies (list strategy0 strategy1)

      ;; Update communality based on PD game outcome
      ask gamer0 [
        set communality PD-game strategy0 strategy1
      ]
      ask gamer1 [
        set communality PD-game strategy1 strategy0
      ]
    ][
      ;; No PD game this round
      set had-game? false
      set used-strategies ["N" "N"]
    ]
  ]
...
to-report cut-link [who0 who1]
  ...
  ifelse ([had-game?] of link who0 who1)[
    let opponent-strategy item 1 [used-strategies] of link who0 who1
    ifelse opponent-strategy = "C"[
      report false
    ][
      report true
    ]
